    ...
    <main className="ritual-texture min-h-screen px-4 py-10 text-[#241b18] sm:px-6">
      <article className="mx-auto max-w-3xl rounded-lg border border-[#dfccb0] bg-[#fffaf2] p-6 shadow-[0_24px_70px_rgba(66,48,31,0.12)] sm:p-10">
        ...
        <span className="mt-10 grid h-12 w-12 place-items-center rounded-2xl border border-[#dfccb0] bg-white/70">
          <Image
            src={visual}
            alt={t(props.visualAlt ?? props.title)}
            width={34}
            height={34}
        ...
        <p className="mt-5 text-base leading-7 text-[#6f615a]">{t(props.intro)}</p>
        <p className="mt-3 text-xs text-[#8a786b]">{t("Atualizado em 13 de junho de 2026.")}</p>

        <div className="mt-10 space-y-8">
          {props.sections.map((section) => (
            <section key={section.title}>
              <h2 className="brand-serif text-2xl font-semibold">{t(section.title)}</h2>
              <p className="mt-3 text-sm leading-7 text-[#5f5149]">{t(section.body)}</p>
            </section>
          ))}
        </div>
      </article>
